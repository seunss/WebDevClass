...
$(document).ready( function() { // Make sure the documenet is ready
for(let i = 0; i <images.length; i++){ // Traversing through the Images
    var $id = $("<img>", {id: images[i].id, src: "images/square/"+images[i].path, alt: images[i].title }); // Creating Image element object

    $(".gallery").append($id); // add the Image to the list
    
    $( "#"+images[i].id ).mouseenter(function() { // Gray the photot and set up the div for a large image
      $( this ).attr( {"class": "gray"} );
      $("body").append('<div id="preview">');
      $("#preview").html( '<img id="class">' +"<p>" + images[i].title + "</br>" + images[i].city + ", "+ images[i].country + " [" + images[i].taken + "]" + "</p>");
      $( "#class").attr({ src: "images/medium/"+images[i].path, "alt": images[i].title});

      
      $("#preview").css({"display":"block","left": (event.pageX +5) +"px", "top": (event.pageY +5) +"px"}); // setting the postion of the photo

    });

    $("#"+ images[i].id).mousemove(function(){ // Keeping the box in the same spot
      this.attr({"left": (event.pageX +5) +"px", "top": (event.pageY +5) +"px"})
    });


   
      $( "#"+images[i].id ).mouseleave(function() { // Getting Rid of the Large Image
